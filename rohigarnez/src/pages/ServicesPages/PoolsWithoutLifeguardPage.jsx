...
export default function PoolsWithoutLifeguardPage({ onNavigate }) {
    ...
    const fetchData = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/api/services/poolswithoutlifeguardpage`
        )
                  ...
                  {data?.content?.benefits?.map((benefit, index) => {
                    ...
                    return (
                      <div key={index} className="flex gap-6 p-8 border-b last:border-0 border-[#141516]/5 hover:bg-white transition-colors group">
                        {Icon && <Icon className="h-6 w-6 text-[#70a2ad] stroke-[1.5] group-hover:text-[#141516] transition-colors" />}
                        <div>
