...
export default function PipeRepairPage({ onNavigate }) {
    ...
    const fetchData = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/api/services/piperepairpage`
        )
      ...
      <section className="py-24 bg-white relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 border-t border-l border-[#141516]/10">
            {data?.content?.problems?.map((problem, index) => (
              <div key={index} className="p-8 border-r border-b border-[#141516]/10 hover:bg-[#f8f9fa] transition-colors group h-full">
                <div className="flex justify-between items-start mb-6">
                  <AlertCircle className={`w-6 h-6 stroke-[1.5] ${problem.severity === 'high' ? 'text-[#141516]' : 'text-[#70a2ad]'
                    }`} />
                  <span className={`text-[10px] font-mono uppercase tracking-widest px-2 py-1 border ${problem.severity === 'high'
