...
export default function SaltChlorinatorPage({ onNavigate }) {
    ...
    const fetchSaltData = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/api/services/satlchlorinatorpage`
        )
        const data = await res.json();
        setSaltData(data);
      }
      catch (error) {
        console.error('Error cargando SaltChlorinatorPage:', error);
      }
    }
    fetchSaltData();
  }, []);
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-[#141516]/10 border border-[#141516]/10">
            {saltData?.content?.benefits?.map((benefit, i) => {
              ...
              return (
                <div key={i} className="bg-white p-8 hover:bg-gray-50 transition-colors group relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity">
                    {Icon && <Icon className="h-12 w-12 text-[#141516]/5 -rotate-12" />}
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-8">
              <div className="grid grid-cols-1 gap-0">
                {saltData?.content?.installation?.map((step, i) => (
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border border-[#141516]/10 divide-y md:divide-y-0 md:divide-x divide-[#141516]/10">
            {saltData?.content?.systems?.map((sys, i) => {
              return (
                <div key={i} className={`p-8 bg-white relative group flex flex-col justify-between ${sys.recommended ? 'bg-gray-50' : ''}`}>
                  {sys.recommended && (
                  ...
                  <Button
                    ...
                    className={`w-full rounded-none h-12 text-xs uppercase tracking-widest font-medium ${sys.recommended
