...
import { AlertTriangle, Zap, Shield, Activity, Gauge } from 'lucide-react';
import { Button } from '../../components/ui/button';

export default function LeakDetectionPage({ onNavigate }) {
    ...
    const fetchData = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/api/services/leakdetectionpage`
        )
                ...
                {data?.content?.benefits?.map((benefit, index) => {
                  ...
                  return (
                    <div key={index} className="p-10 border-b border-r border-[#141516]/10 hover:bg-gray-50 transition-colors group">
                      {Icon && <Icon className="w-8 h-8 text-[#70a2ad] stroke-[1.5] mb-6 group-hover:scale-110 transition-transform duration-300" />}
      ...
      <section className="py-24 bg-[#f8f9fa] relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-0 border-t border-[#141516]/10">
            {data?.content?.process?.map((item, index) => (
              <div key={index} className="group relative p-8 border-r border-b border-[#141516]/10 hover:bg-white transition-colors last:border-r-0 md:last:border-r">
                ...
                {index < data?.content?.process?.length - 1 && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12">
              ...
              <div className="lg:col-span-8 p-12">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
                  {data?.content?.signals?.map((sign, i) => (
