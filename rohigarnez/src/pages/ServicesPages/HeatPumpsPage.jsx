...
export default function HeatPumpsPage({ onNavigate }) {
    ...
    const fetchData = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/api/services/heatpumpspage`
        )
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border border-[#141516]/10 divide-y md:divide-y-0 md:divide-x divide-[#141516]/10">
            {data?.content?.models?.map((model, i) => (
              <div key={i} className={`p-8 bg-white relative group ${model.popular ? 'bg-gray-50' : ''}`}>
                ...
                <Button
                  ...
                  className={`w-full rounded-none h-12 text-xs uppercase tracking-widest font-medium ${model.popular
