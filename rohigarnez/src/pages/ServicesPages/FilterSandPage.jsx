...
export default function FilterSandPage({ onNavigate }) {
      ...
      const fetchData = async () => {
         try {
            const res = await fetch(
               `${import.meta.env.VITE_API_URL}/api/services/filtersandpage`
            )
         ...
         <section className="py-24 bg-white">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border border-[#141516]/10 divide-y md:divide-y-0 md:divide-x divide-[#141516]/10">
                  {data?.content?.filterMedia?.map((media, i) => (
                     <div key={i} className={`p-10 bg-white relative group ${media.recommended ? 'bg-gray-50' : ''}`}>
                        ...
                        <Button
                           ...
                           className={`w-full rounded-none h-14 text-xs uppercase tracking-widest font-medium ${media.recommended
