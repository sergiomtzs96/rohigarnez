...
export default function AirTestingPage({ onNavigate }) {
    ...
    const fetchData = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/api/services/airtestingpage`
        )
            ...
            <div className="lg:col-span-5">
              ...
              <div className="bg-[#f8f9fa] p-8 border border-[#141516]/5">
                ...
                <ul className="space-y-4">
                  {data?.content?.advantages?.map((adv, i) => (
      ...
      <section className="py-24 bg-[#f8f9fa] relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0 border-t border-[#141516]/10">
            {data?.content?.applications?.map((app, i) => {
              ...
              return (
                <div key={i} className="group p-8 border-r border-b border-[#141516]/10 bg-white hover:bg-[#70a2ad]/5 transition-colors last:border-r-0 lg:last:border-r-0">
                  <div className="mb-6">
                    {Icon && <Icon className="w-8 h-8 text-[#70a2ad] stroke-[1.5] group-hover:scale-110 transition-transform duration-300" />}
                  </div>
