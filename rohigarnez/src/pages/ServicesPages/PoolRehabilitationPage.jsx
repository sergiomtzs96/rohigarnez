...
import { Droplets, Hammer, Palette, Settings } from 'lucide-react';
import { Button } from '../../components/ui/button';



export default function PoolRehabilitationPage({ onNavigate }) {
      ...
      const fetchData = async () => {
         try {
            const res = await fetch(
               `${import.meta.env.VITE_API_URL}/api/services/poolrehabilitationpage`
            )
         ...
         <section className="py-24 bg-[#141516] text-white relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-0 border-l border-white/10">
                  {data?.content?.phases?.map((phase, i) => (
         ...
         <section className="py-24 bg-[#f8f9fa]">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
               ...
               <div className="grid grid-cols-1 lg:grid-cols-3 gap-0 border-t border-x border-[#141516]/10">
                  {data?.content?.packages?.map((pkg, i) => (
                     <div key={i} className={`relative p-10 border-b border-[#141516]/10 bg-white group hover:z-10 transition-all ${pkg.recommended ? 'ring-1 ring-[#70a2ad] z-10' : ''
                        }`}>
                        {pkg.recommended && (
                        ...
                        <div className="mt-10 pt-6 border-t border-[#141516]/5">
                           <Button
                              ...
                              className={`w-full rounded-none h-12 text-xs tracking-[0.15em] uppercase font-bold transition-all ${pkg.recommended
